...
`ip_checker` integrates **optional** OWASP-related workflows via `owasp_toolkit.py`.  
This repository remains **MIT** licensed. OWASP projects listed below are **not vendored** unless you install them separately.

## Components

| Component | Role in ip_checker | Upstream license | How we use it |
|-----------|-------------------|------------------|---------------|
...
- **Apache-2.0 (Amass, Secure Headers project):** Calling an external CLI or implementing checks against public header names does not require changing `ip_checker`'s license. Do not copy large chunks of upstream documentation into this repo without attribution and license compliance.
...
- **CC-BY-SA-4.0 (WSTG):** Linking and short summaries is fine; do not paste large sections of WSTG into `ip_checker` without CC-BY-SA attribution and ShareAlike considerations.
