...
**На диске:** `fnkit.py` и `paths.py` / `schema.py` в корне · Python-модули в `lib/` · базы, конфиг и сессии в `data/` (старые пути из корня переносятся при первом запуске). Подробнее о версиях схем: [SCHEMA.md](docs/SCHEMA.md).
